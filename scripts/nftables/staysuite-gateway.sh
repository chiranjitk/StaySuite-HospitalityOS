...
#
#  Equivalent of Cryptsk's defaultchains.sh, rewritten for nftables
#  on Rocky Linux 10.
#
...
# ─── Portal ports (Cryptsk: 8888 HTTP, 8443 HTTPS) ────────────────────
...
# ─── Captive portal redirect (Cryptsk: mark 10000=HTTP, 20000=HTTPS) ─
MARK_HTTP_REDIRECT=10000
MARK_HTTPS_REDIRECT=20000
MARK_AUTHENTICATED=30000

# ─── Rate limiting for unauthenticated users (Cryptsk: 230/min) ───────
...
#  (Cryptsk equivalent: iptables -t mangle -F, iptables -X, etc.)
# ═══════════════════════════════════════════════════════════════════════════
...
#  (Cryptsk equivalent: creating mangle, nat, filter tables implicitly)
# ═══════════════════════════════════════════════════════════════════════════
...
#  (Cryptsk equivalent: ipset -N loggedinusers hash:net, usersset list:set, etc.)
# ═══════════════════════════════════════════════════════════════════════════
...
create_sets() {
    ...
    # (Cryptsk: ipset -N loggedinusers hash:net)
    ...
    # (Cryptsk: ipset -N loggedinusersnetwork hash:net)
    ...
    # (Cryptsk: ipset -N loggedinusersdstip hash:net)
    ...
    # (Cryptsk: ipset -N denynetwork bitmap:ip,mac range ...)
...
#  (Cryptsk equivalent: iptables -t mangle -N open, accountingup, acctup, etc.)
# ═══════════════════════════════════════════════════════════════════════════
...
create_chains() {
    ...
    # (Cryptsk: accountingup chain with mark 10000/20000)
    ...
    # (Cryptsk: iptables -t nat -I PREROUTING -m mark --mark 10000 -j REDIRECT --to-ports 8888)
...
#  (Cryptsk equivalent: iptables -I open -s 0/0 -d 127.0.0.1 -j ACCEPT, etc.)
# ═══════════════════════════════════════════════════════════════════════════
...
add_open_rules() {
    ...
    # ─── ICMP (ping) ──────────────────────────────────────────────────
    nft add rule inet staysuite_mangle open ip protocol icmp accept
    nft add rule inet staysuite_mangle open ip6 nexthdr icmpv6 accept
	
	nft insert rule inet staysuite_filter input tcp dport 22 ct state new,established accept
	nft insert rule inet staysuite_filter output tcp sport 22 ct state established accept
    ...
    # (Cryptsk: iptables -A PREROUTING -s 0/0 -d 0/0 -j open -t mangle)
...
#  (Cryptsk equivalent: loop over ifcfg-eth* files, check nettype, add rules)
#
#  Key logic:
...
add_interface_rules() {
    ...
    for conn_file in "$NM_CONNS_DIR"/*.nmconnection; do
        ...
        # (Cryptsk: nettype=0 LAN, nettype=2 VLAN → captive portal redirect)
        case "$nettype" in
...
# ─── LAN Interface Rules ────────────────────────────────────────────────
# (Cryptsk: the ifcfg-eth* loop for nettype=0 and nettype=2)
#
# Key logic from Cryptsk:
...
add_lan_interface_rules() {
    ...
    # (Cryptsk: iptables -I open -s $ipaddr -d 0/0 -j ACCEPT -t mangle)
    ...
    # (Cryptsk: iptables -I open -s 0/0 -d $ipaddr -m multiport -p tcp --dport 8007,8009,3306,389,3128 -j DROP)
    ...
    # (Cryptsk: iptables -A open -t mangle -s 0/0 -d 0/0 -m multiport -p tcp --dport 137,138,139,445 -j DROP)
    nft add rule inet staysuite_mangle open \
    ...
    #
    # (Cryptsk: the accountingup chain with mark 10000/20000)

    # --- Rate limit HTTP for unauthorized users ---
    # (Cryptsk: iptables -A accountingup -p tcp --dport 80 -s $ipaddr/$netmask
    ...
    # (Cryptsk: iptables -A accountingup -p tcp --dport 443 ... mark 20000)
    nft add rule inet staysuite_mangle unauth_redirect \
    ...
    # (Cryptsk: iptables -A accountingup -p tcp -s $ipaddr/$netmask -d 0/0
    ...
    # (Cryptsk: iptables -A accountingup -s $ipaddr/$netmask -d 0/0 -m state --state NEW -j DROP)
    nft add rule inet staysuite_mangle unauth_redirect \
        iifname "$device" \
        ct state new \
        drop
...
# ─── WAN Interface Rules ────────────────────────────────────────────────
# (Cryptsk: nettype=1 interfaces get MASQUERADE + restricted input)
...
add_wan_interface_rules() {
    ...
    # (Cryptsk: implicit via NAT + routing)
...
#  (Cryptsk equivalent: iptables -t nat -I PREROUTING -m mark --mark 10000
...
add_captive_portal_redirect() {
    ...
    # (Cryptsk: iptables -t nat -I PREROUTING -m mark --mark 10000 -j REDIRECT -p tcp --to-ports 8888)
    nft add rule inet staysuite_nat portal_redirect \
    meta mark 10000 tcp dport 80 \
    dnat to :3080 \
    ...
    # (Cryptsk: iptables -t nat -I PREROUTING -m mark --mark 20000 -j REDIRECT -p tcp --to-ports 8443)
    nft add rule inet staysuite_nat portal_redirect \
	'meta mark 20000 tcp dport 443 dnat to :3443 comment "StaySuite HTTPS captive portal redirect"'
    ...
    # (Cryptsk: iptables -t nat -I PREROUTING -j open)
...
#  (Cryptsk equivalent: iptables -t mangle -I PREROUTING -m set --match-set
...
add_auth_passthrough() {
    ...
    # (Cryptsk: iptables -t mangle -I PREROUTING -m set --match-set usersset src,src -j ACCEPT)
    ...
    # (Cryptsk: iptables -t mangle -I PREROUTING -m set --match-set usersset src,src -j CONNMARK --save-mark)
    ...
    # (Cryptsk: iptables -t mangle -I PREROUTING -m set --match-set usersset src,src -j CONNMARK --restore-mark)
    nft add rule inet staysuite_mangle prerouting \
        ip saddr @authenticated_users \
        meta mark set ct mark
    ...
    # (Cryptsk: iptables -t mangle -I PREROUTING -m set --match-set usersset src,src -m connmark ! --mark 0 -j ACCEPT)
    nft add rule inet staysuite_mangle prerouting \
        ip saddr @authenticated_users \
        ct mark != 0 \
        accept
    ...
    # (Cryptsk: iptables -I POSTROUTING -f -j DROP -t mangle)
    #nft add rule inet staysuite_mangle postrouting \
	#'ip frag-off != 0 drop'
...
#  (Cryptsk equivalent: iptables -t mangle -A PREROUTING -s 0/0 -d 0/0
...
add_accounting() {
    ...
    # (Cryptsk: iptables -I accountingup -m connmark ! --mark 0 -j DROP -t mangle)
    nft add rule inet staysuite_mangle prerouting \
        ct mark == 0 \
        drop
...
#  (Cryptsk equivalent: ipset -N gw${gatewayid}ipset + fwmark per gateway)
# ═══════════════════════════════════════════════════════════════════════════
...
#  (Cryptsk equivalent: /usr/local/scripts/general_settings.sh,
...
add_security_rules() {
    ...
    nft add rule inet staysuite_filter forward ct state invalid drop

    # ─── Drop fragmented packets ───────────────────────────────────────
    # nft add rule inet staysuite_filter input ip frag-offload & 0x1fff != 0 drop

    # ─── SMB logging (Cryptsk: LOG --log-prefix "CRYPT_NETSEC") ─────
    nft add rule inet staysuite_filter input \
...
#  (Cryptsk equivalent: /etc/denynetwork file + generate_denynetwork_file.sh)
# ═══════════════════════════════════════════════════════════════════════════
...
#  (Cryptsk equivalent: /usr/local/scripts/blockmacentries.sh)
# ═══════════════════════════════════════════════════════════════════════════
...
#  (Cryptsk equivalent: the LAST rule in the chain)
# ═══════════════════════════════════════════════════════════════════════════
...
add_final_drop() {
    ...
    # (Cryptsk: implicitly handled by chain order)
...
main() {
    ...
    # Execute steps in order (Cryptsk script order preserved)

...
function getSystemInfo() {
  try {
    ...
    let kernel = '';
    let osRelease = 'StaySuite Hospitality OS';
    let osVersion = '';
    let uptimeFormatted = '';
    let loadAverage = '';
    ...
    try {
      const versionId = execSync("cat /etc/os-release 2>/dev/null | grep VERSION_ID | cut -d'=' -f2 | tr -d '\"'", { encoding: 'utf-8' }).trim();
      if (versionId) osVersion = versionId;
    } catch { /* fallback */ }

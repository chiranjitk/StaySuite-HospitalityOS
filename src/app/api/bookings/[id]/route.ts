...
export async function PUT(
  ...
  try {
    ...
    if (status === 'checked_in' && effectiveRoomId) {
      ...
      // Emit booking checked-in event (triggers WiFi provisioning)
      try {
        ...
        // Get WiFi credentials for the response
        const { wifiProvisioningService } = await import('@/lib/wifi/services/provisioning-service');
        const credentials = await wifiProvisioningService.getWiFiCredentialsForBooking(booking.id);
...
export async function PATCH(
  ...
  try {
    ...
    if (status === 'checked_in' && effectivePatchRoomId) {
      ...
      // FIX 3: Emit booking checked-in event (triggers WiFi provisioning) — matching PUT handler
      try {
        await emitBookingCheckedIn({
          bookingId: booking.id,
          tenantId: booking.tenantId,

...
import { format, addDays, eachDayOfInterval, subDays, getDay, getMonth } from 'date-fns';
  ...
  try {
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        avgPredictedOccupancy: Math.round(forecastData.reduce((sum, d) => sum + d.predicted, 0) / forecastData.length),
        peakDays: peakDays.map(d => d.date),
        lowDays: lowDays.map(d => d.date),
        seasonalFactor: monthlyFactors[getMonth(new Date())] || 1,
        trendFactor: Math.round(trendFactor * 100) / 100,
        dataPoints: bookings.length,
        hasSufficientData: bookings.length >= 30,
      },
    });
  } catch (error) {

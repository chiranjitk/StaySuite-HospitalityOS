                    ...
                    <div className="grid bg-muted/50 border-b" style={{ gridTemplateColumns: `140px repeat(${calendarDays.length}, minmax(40px, 1fr))` }}>
                      ...
                      {calendarDays.map((day) => (
                        ...
                        >
                          ...
                          <div className={cn(isWeekend(day) && 'text-amber-600 dark:text-amber-400')}>{format(day, 'd')}</div>
                        </div>
                      ))}
                    </div>
                    ...
                    {ratePlans.map((plan) => (
                      <Card key={plan.id} className="p-3">
                          ...
                          <span className="font-medium">
                            {plan.hasActivePromo && plan.effectivePrice ? (
                              <>
                                ...
                                <span className="text-emerald-600 dark:text-emerald-400">{formatCurrency(plan.effectivePrice)}</span>
                              </>
                            ) : (
                              formatCurrency(plan.basePrice)
                            )}
                        ...
                        <div className="flex gap-2 mt-3 pt-2 border-t">
                          ...
                          <Button variant="outline" size="sm" className="h-9 min-h-[44px] text-red-600 dark:text-red-400" onClick={() => deleteRatePlan(plan.id)}>

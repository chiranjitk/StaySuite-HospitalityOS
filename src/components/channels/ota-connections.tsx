...
const getStatusBadge = (status: string) => {
  const variants: Record<string, { variant: 'default' | 'secondary' | 'destructive' | 'outline'; className: string }> = {
    ...
    suspended: { variant: 'outline', className: 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400' },
  };
...
const getPriorityBadge = (priority: string) => {
  const variants: Record<string, { className: string; label: string }> = {
    critical: { className: 'bg-red-100 text-red-700 dark:text-red-300 border-red-200', label: 'Critical' },
    high: { className: 'bg-orange-100 text-orange-700 dark:text-orange-300 border-orange-200', label: 'High' },
    medium: { className: 'bg-yellow-100 text-yellow-700 dark:text-yellow-300 border-yellow-200', label: 'Medium' },
    low: { className: 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300 border-gray-200 dark:border-gray-700', label: 'Low' },
  };
        ...
        <ScrollArea className="flex-1 px-4 overflow-y-auto" style={{ height: 'calc(95vh - 180px)' }}>
          {!selectedChannel ? (
            ...
            <div className="space-y-4 py-4">
              ...
              {/* Test Result */}
              {testResult && (
                <Alert variant={testResult.success ? 'default' : 'destructive'} className={testResult.success ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-950/30' : ''}>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      {connection.lastError && (
                        <div className="p-2 rounded-md bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800">
                          <p className="text-xs text-red-600 dark:text-red-400 truncate">{connection.lastError}</p>
                        </div>
                      )}
                      ...
                      <div className="grid grid-cols-2 gap-2 text-center">
                        ...
                        <div className="p-2 rounded-md bg-red-50 dark:bg-red-950/20">
                          <p className="text-lg font-semibold text-red-600 dark:text-red-400">{connection.failedSyncs || 0}</p>
                      ...
                      <div className="flex items-center justify-between pt-2 border-t">
                        <div className="flex items-center gap-2">
                          <Zap className={`h-4 w-4 ${connection.autoSync ? 'text-amber-500 dark:text-amber-400' : 'text-muted-foreground'}`} />

...
export default function FeedbackReviews() {
        ...
        {[1, 2, 3, 4, 5].map((star) => (
          <Star
            ...
            className={`${starSize} transition-transform duration-150 ${
              star <= rating
                ? 'text-amber-400 dark:text-amber-300 fill-amber-400 drop-shadow-sm'
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case 'complaint': return <AlertCircle className="h-4 w-4 text-red-500 dark:text-red-400" />;
      case 'compliment': return <ThumbsUp className="h-4 w-4 text-emerald-500 dark:text-emerald-400" />;
      case 'suggestion': return <MessageSquare className="h-4 w-4 text-cyan-500 dark:text-cyan-400" />;
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Star className="h-5 w-5 text-amber-400 dark:text-amber-300 fill-amber-400" />
                </div>
              </div>
            </div>
          </CardContent>

...
export function MiniCalendarWidget() {
  ...
  const events: CalendarEvent[] = useMemo(() => {
    ...
    // Generate realistic mock events spread across the month
    const checkInDays = [3, 5, 7, 10, 12, 14, 15, 17, 19, 21, 23, 25, 27, 28];
    const checkOutDays = [4, 6, 9, 11, 13, 16, 18, 20, 22, 24, 26, 29];
    const maintenanceDays = [8, 22];
    const eventDays = [15, 20];
    
    checkInDays.forEach(day => {
      if (day <= daysInMonth) {
        evts.push({ day, type: 'checkin', count: Math.floor(Math.random() * 8) + 2, label: 'Check-ins' });
      }
    });
    checkOutDays.forEach(day => {
      if (day <= daysInMonth) {
        evts.push({ day, type: 'checkout', count: Math.floor(Math.random() * 6) + 1, label: 'Check-outs' });
      }
    });
    maintenanceDays.forEach(day => {
      if (day <= daysInMonth) {
        evts.push({ day, type: 'maintenance', count: 2, label: 'Maintenance' });
      }
    });
    eventDays.forEach(day => {
      if (day <= daysInMonth) {
        evts.push({ day, type: 'event', count: 1, label: 'Event' });
      }
    });
            ...
            return (
              <motion.button
                ...
                className={cn(
                  ...
                  isSelected && !dayIsToday && 'bg-primary/10 ring-1 ring-primary/30',
        ...
        <AnimatePresence mode="wait">
          {selectedDay && selectedDayEvents.length > 0 && (
            <motion.div
              ...
              className="space-y-1.5 pt-2 border-t border-border/30 overflow-hidden"
            >
          ...
          {selectedDay && selectedDayEvents.length === 0 && (
            <motion.div
              ...
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="text-center py-2 border-t border-border/30"
            >

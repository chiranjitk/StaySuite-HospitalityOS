...
const TYPE_CONFIG: Record<EventType, { label: string; color: string; bg: string; border: string; icon: React.ElementType }> = {
  conference: { label: 'Conference', color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-100 dark:bg-blue-900/40', border: 'border-blue-300 dark:border-blue-700', icon: Monitor },
  wedding: { label: 'Wedding', color: 'text-rose-600 dark:text-rose-400', bg: 'bg-rose-100 dark:bg-rose-900/40', border: 'border-rose-300 dark:border-rose-700', icon: CalendarHeart },
  meeting: { label: 'Meeting', color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-100 dark:bg-amber-900/40', border: 'border-amber-300 dark:border-amber-700', icon: UsersRound },
  banquet: { label: 'Banquet', color: 'text-violet-600 dark:text-violet-400', bg: 'bg-violet-100 dark:bg-violet-900/40', border: 'border-violet-300 dark:border-violet-700', icon: GlassWater },
  private: { label: 'Private', color: 'text-teal-600 dark:text-teal-400', bg: 'bg-teal-100 dark:bg-teal-900/40', border: 'border-teal-300 dark:border-teal-700', icon: Lock },
};

const STATUS_CONFIG: Record<EventStatus, { label: string; className: string }> = {
  confirmed: { label: 'Confirmed', className: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800' },
  pending: { label: 'Pending', className: 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400 border-amber-200 dark:border-amber-800' },
  cancelled: { label: 'Cancelled', className: 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400 border-red-200 dark:border-red-800' },
};
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        countdown.isToday
          ? 'border-amber-300 dark:border-amber-700 ring-2 ring-amber-400/40 dark:ring-amber-600/40 bg-gradient-to-br from-amber-50/60 to-transparent dark:from-amber-950/50'

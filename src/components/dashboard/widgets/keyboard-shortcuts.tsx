...
const shortcuts: ShortcutItem[] = [
  { keys: ['Ctrl', 'K'], label: 'Search / Command Palette', icon: Search, color: 'text-teal-600' },
  { keys: ['Ctrl', 'N'], label: 'New Booking', icon: Plus, color: 'text-emerald-600' },
  { keys: ['Ctrl', 'R'], label: 'Refresh Dashboard', icon: RefreshCw, color: 'text-sky-600' },
  { keys: ['Ctrl', '\\'], label: 'Toggle Sidebar', icon: PanelLeft, color: 'text-violet-600' },
  { keys: ['Ctrl', '.'], label: 'Notifications', icon: Bell, color: 'text-amber-600' },
  { keys: ['Ctrl', 'Enter'], label: 'Quick Action', icon: ArrowRight, color: 'text-rose-600' },
];
                  ...
                  {shortcut.keys.map((key, ki) => (
                    <React.Fragment key={key}>
                      ...
                      <kbd className={cn(
                        ...
                        "group-hover:border-primary/30 group-hover:text-foreground transition-colors"
                      )}>
                        {key}
                      </kbd>
                    </React.Fragment>

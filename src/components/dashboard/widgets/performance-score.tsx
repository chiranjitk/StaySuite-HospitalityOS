...
const GRADE_CONFIG: Record<string, { label: string; color: string; bg: string; ring: string }> = {
  'A+': { label: 'Exceptional', color: 'text-emerald-600', bg: 'bg-emerald-50 dark:bg-emerald-950/30', ring: 'ring-emerald-200 dark:ring-emerald-800' },
  'A': { label: 'Excellent', color: 'text-emerald-600', bg: 'bg-emerald-50 dark:bg-emerald-950/30', ring: 'ring-emerald-200 dark:ring-emerald-800' },
  'B+': { label: 'Very Good', color: 'text-teal-600', bg: 'bg-teal-50 dark:bg-teal-950/30', ring: 'ring-teal-200 dark:ring-teal-800' },
  'B': { label: 'Good', color: 'text-sky-600', bg: 'bg-sky-50 dark:bg-sky-950/30', ring: 'ring-sky-200 dark:ring-sky-800' },
  'C+': { label: 'Above Average', color: 'text-amber-600', bg: 'bg-amber-50 dark:bg-amber-950/30', ring: 'ring-amber-200 dark:ring-amber-800' },
  'C': { label: 'Average', color: 'text-amber-600', bg: 'bg-amber-50 dark:bg-amber-950/30', ring: 'ring-amber-200 dark:ring-amber-800' },
  'D': { label: 'Below Average', color: 'text-red-600', bg: 'bg-red-50 dark:bg-red-950/30', ring: 'ring-red-200 dark:ring-red-800' },
};
...
export function PerformanceScoreWidget() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (result.success) {
        ...
        const metrics: PerformanceMetric[] = [
          ...
          },
          {
            ...
            trendValue: Math.round(Math.random() * 12 + 2),
          },
          {
            label: 'Guest Sat.',
            score: 86,
          ...
          },
          {
            ...
            trendValue: Math.round(Math.random() * 5 + 1),
          },
        ];
        ...
        const gradeColor = GRADE_CONFIG[grade]?.color || 'text-amber-600';
      ...
      const metrics: PerformanceMetric[] = [
        { label: 'Occupancy', score: 78, maxScore: 100, weight: 30, icon: Bed, color: 'text-violet-600', trend: 'up', trendValue: 5 },
        { label: 'Revenue', score: 82, maxScore: 100, weight: 25, icon: DollarSign, color: 'text-emerald-600', trend: 'up', trendValue: 8 },
        { label: 'Guest Sat.', score: 86, maxScore: 100, weight: 25, icon: Star, color: 'text-amber-600', trend: 'up', trendValue: 3 },
        { label: 'Operations', score: 91, maxScore: 100, weight: 20, icon: Zap, color: 'text-cyan-600', trend: 'up', trendValue: 2 },
      ];
      const overallScore = Math.round(metrics.reduce((sum, m) => sum + (m.score * m.weight / 100), 0));
      const grade = getGrade(overallScore);
      setData({ overallScore, grade, gradeColor: GRADE_CONFIG[grade]?.color || 'text-amber-600', metrics, lastUpdated: new Date().toLocaleTimeString() });
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  return (
    <Card className="border-0 shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 rounded-2xl overflow-hidden">
      ...
      <CardContent className="space-y-4">
        ...
        {/* Last updated */}
        <p className="text-[10px] text-muted-foreground/50 text-center pt-1 border-t border-border/30">

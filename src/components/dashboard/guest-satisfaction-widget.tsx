...
function ratingBgClass(score: number | null) {
  const s = score ?? 0;
  if (s >= 4) return 'bg-emerald-50 dark:bg-emerald-900/20';
  if (s >= 3) return 'bg-amber-50 dark:bg-amber-900/20';
...
function ReviewSnippet({ review }: { review: Review }) {
  ...
  return (
    <motion.div
      ...
      className="group p-2.5 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors"
    >

        ...
        <div className="absolute bottom-3 left-3 right-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1 bg-white/20 backdrop-blur-sm rounded-full px-2 py-0.5">
              <Star className="h-3 w-3 text-yellow-300 dark:text-yellow-200 fill-yellow-300" />
                ...
                {themes.find(t => t.id === themeId)?.features && (
                  <>
                    {themes.find(t => t.id === themeId)!.features.glassEffect && (
                      <span className="px-3 py-1.5 rounded-full bg-blue-500/10 text-blue-500 dark:text-blue-400 text-sm font-medium border border-blue-500/20">
                        Glass Effect
                      </span>
                    )}
                    {themes.find(t => t.id === themeId)!.features.softShadows && (
                      <span className="px-3 py-1.5 rounded-full bg-purple-500/10 text-purple-500 dark:text-purple-400 text-sm font-medium border border-purple-500/20">
                        Soft Shadows
                      </span>
                    )}
                    {themes.find(t => t.id === themeId)!.features.gradients && (
                      <span className="px-3 py-1.5 rounded-full bg-pink-500/10 text-pink-500 dark:text-pink-400 text-sm font-medium border border-pink-500/20">
                        Gradients
                      </span>
                    )}
                    {themes.find(t => t.id === themeId)!.features.animations && (
                      <span className="px-3 py-1.5 rounded-full bg-green-500/10 text-green-500 dark:text-green-400 text-sm font-medium border border-green-500/20">
                        Animations
                      </span>
                    )}
                    <span className="px-3 py-1.5 rounded-full bg-amber-500/10 text-amber-600 dark:text-amber-400 text-sm font-medium border border-amber-500/20">

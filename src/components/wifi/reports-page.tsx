...
function CircularGauge({ value, label, color, size = 120 }: { value: number; label: string; color: string; size?: number }) {
  ...
  const colorMap: Record<string, { stroke: string; text: string }> = {
    teal: { stroke: 'stroke-teal-500', text: 'text-teal-600' },
...
type TabId = 'bandwidth' | 'user-bw' | 'web-surfing' | 'nat-logs' | 'syslog' | 'sys-health';
  ...
  const fetchBandwidth = useCallback(async (range: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({ title: 'Error', description: result.error || 'Failed to fetch bandwidth data', variant: 'destructive' });
      }
    } catch (e) {
      console.error(e);
                    ...
                    >
                      ...
                      {hoveredBar === i && (
                        <div className="absolute -top-16 left-1/2 -translate-x-1/2 bg-popover border rounded-lg p-2 shadow-lg text-xs z-50 pointer-events-none whitespace-nowrap">
                          ...
                          <p className="text-teal-600">↓ {formatMB(day.download)}</p>
                          <p className="text-amber-600">↑ {formatMB(day.upload)}</p>
                        </div>
                      )}
                    </div>
                  </TooltipTrigger>
              ...
              <TableBody>
                {filteredData.map((day) => (
                  <TableRow key={day.date} className="hover:bg-muted/30">
                    ...
                    <TableCell className="text-right text-teal-600 font-mono text-sm">{day.download.toLocaleString()}</TableCell>
                    <TableCell className="text-right text-amber-600 font-mono text-sm">{day.upload.toLocaleString()}</TableCell>
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({ title: 'Error', description: result.error || 'Failed to fetch user bandwidth', variant: 'destructive' });
      }
    } catch (e) {
      console.error(e);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base flex items-center gap-2"><Download className="h-4 w-4 text-teal-500" /> Top 10 by Download</CardTitle>
          </CardHeader>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base flex items-center gap-2"><Upload className="h-4 w-4 text-amber-500" /> Top 10 by Upload</CardTitle>
          </CardHeader>
          <CardContent className="space-y-1.5">
              ...
              <div key={plan} className="rounded-lg border p-3">
                ...
                <div className="mt-2 space-y-1">
                  <div className="flex justify-between text-xs">
                    <span className="text-teal-600">↓ {formatMB(data.avgDown)}</span>
                    <span className="text-amber-600">↑ {formatMB(data.avgUp)}</span>
                  </div>
                ...
                {filteredUsers.map((user) => (
                  <React.Fragment key={user.username}>
                    ...
                    >
                      ...
                      <TableCell className="text-right text-teal-600 font-mono text-sm">{formatMB(user.totalDown)}</TableCell>
                      <TableCell className="text-right text-amber-600 font-mono text-sm">{formatMB(user.totalUp)}</TableCell>
                              ...
                              <TableBody>
                                {user.sessionHistory.map((s: any) => (
                                  <TableRow key={s.id}>
                                    ...
                                    <TableCell className="text-xs text-right text-teal-600 font-mono">{formatMB(s.download)}</TableCell>
                                    <TableCell className="text-xs text-right text-amber-600 font-mono">{formatMB(s.upload)}</TableCell>
...
function WebSurfingTab() {
  ...
  const fetchWebSurfing = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({ title: 'Error', description: result.error || 'Failed to fetch web surfing logs', variant: 'destructive' });
      }
    } catch (e) {
      console.error(e);
  ...
  const catColors: Record<string, string> = {
    social_media: 'text-violet-600 bg-violet-100 dark:bg-violet-900/30',
    streaming: 'text-rose-600 bg-rose-100 dark:bg-rose-900/30',
    news: 'text-teal-600 bg-teal-100 dark:bg-teal-900/30',
    gaming: 'text-emerald-600 bg-emerald-100 dark:bg-emerald-900/30',
              ...
              <TableBody>
                {filteredLogs.slice(0, 50).map((log) => (
                  <TableRow key={log.id} className="hover:bg-muted/30">
                    ...
                    <TableCell>
                      <Badge variant={log.action === 'allowed' ? 'default' : 'destructive'} className={cn('text-xs', log.action === 'allowed' && 'bg-emerald-600')}>
                        {log.action}
                      </Badge>
                    </TableCell>
                  </TableRow>
  ...
  const fetchNATLogs = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({ title: 'Error', description: result.error || 'Failed to fetch NAT logs', variant: 'destructive' });
      }
    } catch (e) {
      console.error(e);
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-medium">
                Showing <span className="text-teal-600 font-bold">{filteredLogs.length.toLocaleString()}</span> of <span className="font-bold">{logs.length.toLocaleString()}</span> total entries
              </span>
            </div>
            <div className="flex items-center gap-2">
                ...
                {filteredLogs.slice(0, 100).map((log) => (
                  <TableRow key={log.id} className={cn('hover:bg-muted/30', log.action === 'deny' && 'bg-red-50/50 dark:bg-red-950/10')}>
                    ...
                    <TableCell className="font-mono text-xs">
                      <span className="text-teal-600">{log.sourceIp}</span>:<span className="text-muted-foreground">{log.sourcePort}</span>
                    </TableCell>
                    <TableCell className="font-mono text-xs">
                      <span className="text-amber-600">{log.destIp}</span>:<span className="text-muted-foreground">{log.destPort}</span>
                    </TableCell>
                    <TableCell>
  ...
  const fetchSyslogServers = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({ title: 'Error', description: result.error || 'Failed to fetch syslog config', variant: 'destructive' });
      }
    } catch (e) {
      console.error(e);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({ title: 'Error', description: result.error || 'Failed to save server', variant: 'destructive' });
      }
    } catch (e) {
      console.error(e);
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (result.success) {
        ...
        toast({ title: 'Error', description: result.error || 'Failed to delete server', variant: 'destructive' });
      }
    } catch (e) {
      console.error(e);
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className={cn('p-2 rounded-lg', moduleEnabled ? 'bg-emerald-500/10' : 'bg-gray-500/10')}>
                <Radio className={cn('h-5 w-5', moduleEnabled ? 'text-emerald-500' : 'text-gray-400')} />
              </div>
              <div>
          ...
          <Card key={server.id} className="p-4">
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="flex items-center gap-1">
                ...
                <Button variant="ghost" size="icon" className="h-7 w-7 text-red-500 hover:text-red-600" onClick={() => handleDelete(server.id)}>
      ...
      <Card>
        ...
        <CardContent>
          <div className="rounded-lg bg-gray-950 p-4 font-mono text-xs text-green-400 space-y-1 max-h-40 overflow-y-auto">
  ...
  const fetchHealth = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({ title: 'Error', description: result.error || 'Failed to fetch system health', variant: 'destructive' });
      }
    } catch (e) {
      console.error(e);
              ...
              return (
                <div key={svc.name} className="rounded-lg border p-3 flex items-start gap-3">
                  <div className="mt-0.5">
                    <StatusIcon className={cn('h-5 w-5', isRunning ? 'text-emerald-500' : 'text-red-500')} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
              ...
              <div key={iface.name} className="rounded-lg border p-3">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-3 text-xs">
                    <span className="text-teal-600">↓ {formatBytes(iface.rx)}</span>
                    <span className="text-amber-600">↑ {formatBytes(iface.tx)}</span>
                  </div>
                </div>
                {/* Sparkline */}

...
export default function Invoices() {
  ...
  const StatusBadge = ({ status }: { status: string }) => {
    ...
    const Icon = cfg.icon;
    return (
      <Badge variant="outline" className={cn('gap-1 font-medium border shadow-sm', cfg.gradient ? cfg.gradient : cn(cfg.bgColor, cfg.color))}>
                          ...
                          <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-48">
                                ...
                                {!['paid', 'cancelled'].includes(invoice.status) && (
                                  <DropdownMenuItem onClick={() => cancelInvoice(invoice)} className="text-red-600 dark:text-red-400">
                                    <Ban className="h-4 w-4 mr-2" />Cancel Invoice
                                  </DropdownMenuItem>
                                )}
                                <DropdownMenuItem onClick={() => deleteInvoice(invoice)} className="text-red-600 dark:text-red-400">
                  ...
                  <Card key={item.id} className="p-3">
                    <div className="grid grid-cols-12 gap-2 items-start">
                      ...
                      <div className="col-span-1 flex items-end justify-center">
                        <Button variant="ghost" size="icon" className="h-9 w-9 text-muted-foreground hover:text-red-500 dark:text-red-400" onClick={() => removeLineItem(item.id)} disabled={lineItems.length === 1}>
                          <X className="h-4 w-4" />
                        </Button>
                      </div>
                    </div>
            ...
            <Card className="p-4 bg-muted/30">
              <div className="max-w-[220px] ml-auto space-y-1.5">
                ...
                <div className="flex justify-between font-bold text-base">
                  <span>Total</span>
                  <span className="text-emerald-700 dark:text-emerald-300">{formatCurrency(calcSubtotal() + calcTaxes())}</span>
                </div>
              </div>
            </Card>
                  ...
                  <div className="space-y-1.5 text-sm">
                    ...
                    {selectedInvoice.dueAt && (
                      <div className="flex justify-between">
                        ...
                        <span className={new Date(selectedInvoice.dueAt) < new Date() && selectedInvoice.status !== 'paid' ? 'text-red-600 dark:text-red-400 font-medium' : ''}>
              ...
              <Card className="p-4">
                <div className="max-w-[260px] ml-auto space-y-2">
                  ...
                  <div className="flex justify-between font-bold text-lg">
                    <span>Total</span>
                    <span className="text-emerald-700 dark:text-emerald-300">{formatCurrency(selectedInvoice.totalAmount)}</span>
                  </div>
                  {selectedInvoice.status === 'paid' && selectedInvoice.paidAt && (
                    <p className="text-xs text-emerald-600 dark:text-emerald-400 text-right flex items-center justify-end gap-1">

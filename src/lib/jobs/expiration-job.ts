 ...
 * FreeRADIUS queries no longer filter by isActive — they check if the record exists.
...
async function processUserExpiration(
  ...
  await db.$transaction(async (tx) => {
    ...
    // 2. HARD DELETE RadCheck entries (FreeRADIUS will return 'User not found')
    ...
    // 3. HARD DELETE RadReply entries
...
export async function extendUserValidity(
  ...
  await db.$transaction(async (tx) => {
    ...
    // Re-create RadCheck if deleted (hard delete means record won't exist)

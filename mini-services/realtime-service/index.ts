...
interface RadiusSessionUpdatePayload {
  tenantId: string
  event: 'session-start' | 'session-stop' | 'session-update' | 'coa-action' | 'auth-success' | 'auth-fail'
  username?: string
  sessionId?: string
  nasIp?: string
  clientMac?: string
...
// Emit RADIUS session updates to all tenants (tenant-specific routing handled by the event)
export function emitRadiusSessionUpdate(payload: RadiusSessionUpdatePayload) {
  const ts = payload.timestamp || new Date().toISOString()
  const fullPayload = { ...payload, timestamp: ts }
  io.to(`tenant:${payload.tenantId}`).emit('radius:session', fullPayload)
  console.log(`[Emit] RADIUS session update: ${payload.event} - ${payload.username || 'unknown'}`)
}

// Broadcast active sessions count (for dashboard badges)
export function emitRadiusActiveCount(tenantId: string, activeSessions: number, staleSessions: number) {
  io.to(`tenant:${tenantId}`).emit('radius:active-count', {
    activeSessions,
    staleSessions,
    timestamp: new Date().toISOString()
  })

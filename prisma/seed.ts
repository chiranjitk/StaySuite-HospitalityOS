...
async function main() {
  ...
  // Clean addon module data first (has FK deps that block existing cleanup)
  ...
  // Housekeeping data cleanup (order matters due to foreign keys)
  await prisma.staffWorkload.deleteMany({});
  await prisma.taskAssignmentSuggestion.deleteMany({});
  await prisma.task.deleteMany({});
  await prisma.staffSkill.deleteMany({});
  await prisma.asset.deleteMany({});
  await prisma.preventiveMaintenance.deleteMany({});
  // Discount cleanup (before billing due to FK)
  ...
  // ─── WiFi Vouchers ───────────────────────────────────────
  console.log('Seeding WiFi vouchers...');
  try {
    await prisma.wiFiVoucher.createMany({
      data: [
        ...
        },
        {
          ...
          tenantId: 'tenant-1',
          planId: 'wifiplan-2',
          code: 'WIFI-GUEST5-PREMIUM',
          guestId: 'guest-5',
          bookingId: 'booking-4',
          isUsed: true,
          usedAt: new Date(Date.now() - 12 * 60 * 60 * 1000),
    ...
    console.log('5 WiFi vouchers seeded!');
  } catch (e: any) {
    console.log('WiFi vouchers seed note:', e.message);
  }
      ...
      data: [
        {
          ...
          data: JSON.stringify({ bookingId: 'booking-5', guestId: 'guest-4' }),
        ...
        },
        {
          ...
          data: JSON.stringify({ inspectionId: 'ir-2', roomId: 'room-102', score: 78 }),
        ...
        },
        {
          ...
          data: JSON.stringify({ paymentId: 'pay-1', bookingId: 'booking-1', amount: 17990 }),
        ...
        },
        {
          ...
          data: JSON.stringify({ guestId: 'guest-3', bookingId: 'booking-2', loyaltyTier: 'platinum' }),
          link: '/guests/guest-3',
          priority: 'high',
    ...
    await prisma.auditLog.createMany({
      data: [
        ...
        },
        {
          ...
          entityType: 'Booking',
          entityId: 'booking-1',
          newValue: JSON.stringify({ confirmationCode: 'RS-2024-001', guestId: 'guest-1', roomId: 'room-501' }),
          oldValue: null,
          ipAddress: '192.168.1.100',
          userAgent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)',
        ...
        },
        {
          ...
          ipAddress: '192.168.1.100',
          userAgent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)',
          correlationId: 'checkin-flow-001',
          createdAt: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000),
        },
        {
          id: 'audit-5',
